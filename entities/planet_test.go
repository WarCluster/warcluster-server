...
func TestGeneratePlanets(t *testing.T) {
	...
	expected_planets := []Planet{
		Planet{Color{22, 22, 22}, []int{-76, 57}, false, 6, 3, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{1470, 300}, false, 8, 5, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{-689, -200}, false, 3, 1, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{-1051, 648}, false, 2, 8, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{1428, -1363}, false, 3, 1, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{2735, 300}, false, 6, 8, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{2818, -798}, false, 9, 6, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{-322, 3080}, false, 5, 4, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{1547, 3339}, false, 1, 1, start_time.Unix(), 10, 0, "gophie"},
		Planet{Color{22, 22, 22}, []int{-2744, -1065}, false, 4, 6, start_time.Unix(), 10, 0, "gophie"},
	}
...
func TestDatabasePreparationsWithoutAnOwner(t *testing.T) {
	start_time := time.Now()
	planet := Planet{Color{22, 22, 22}, []int{271, 203}, false, 3, 1, start_time.Unix(), 0, 0, ""}
	json_base := "{\"Color\":{\"R\":22,\"G\":22,\"B\":22},\"IsHome\":false,\"Texture\":3,\"Size\":1,\"LastShipCountUpdate\":%v,\"ShipCount\":0,\"MaxShipCount\":0,\"Owner\":\"\"}"
...
func TestDatabasePreparationsWithAnOwner(t *testing.T) {
	start_time := time.Now()
	planet := Planet{Color{22, 22, 22}, []int{271, 203}, true, 3, 1, start_time.Unix(), 0, 0, "gophie"}
	json_base := "{\"Color\":{\"R\":22,\"G\":22,\"B\":22},\"IsHome\":true,\"Texture\":3,\"Size\":1,\"LastShipCountUpdate\":%v,\"ShipCount\":0,\"MaxShipCount\":0,\"Owner\":\"gophie\"}"
	expected_json := fmt.Sprintf(json_base, start_time.Unix())
	expected_key := "planet.271_203"
